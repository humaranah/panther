...
public class PlayerQueue : IPlayerQueue
{
    ...
    private int GetNextPosition()
    {
        ...
        if (RepeatMode == RepeatMode.RepeatCurrent) return _index;
        if (++_index < _tracks.Count) return _index;
        if (RepeatMode == RepeatMode.NoRepeat) return -1;
    ...
    private int GetPreviousPosition()
    {
        ...
        if (RepeatMode == RepeatMode.NoRepeat) return -1;
        if (!IsShuffleEnabled) return _tracks.Count - 1;
        Shuffle();
        return 0;
    }
